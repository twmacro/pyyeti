...
def test_rainflow_3():
    ...
    assert np.allclose(1., rf[:, 0])
    assert np.allclose(0., rf[:, 1])
    assert np.allclose(.5, rf[:, 2])
...
def test_rainflow_4():
    a = np.ones(10)
    a[::2] *= -1
    a[0] *= 2.  # make rainflow not accept first point until the end
    rf, os = cyclecount.rain.rainflow(a, getoffsets=True)
    rf2, os2 = rainflow(a, getoffsets=True)
    assert np.allclose(rf, rf2)
    assert np.all(os2 == os)
...
def test_rainflow_badinput():
    assert_raises(ValueError, cyclecount.rain.rainflow, 1.)
    assert_raises(ValueError, rainflow, 1.)
    assert_raises(ValueError, cyclecount.rain.rainflow,
                  np.random.randn(2, 2))
    assert_raises(ValueError, rainflow,
                  np.random.randn(2, 2))


def test_getbins_1():
    bb = cyclecount.getbins(4, 12, 4)
    assert np.allclose(bb, [3.992, 6., 8., 10., 12.])
    bb = cyclecount.getbins(4, 4, 12)
    assert np.allclose(bb, [3.992, 6., 8., 10., 12.])
    bb = cyclecount.getbins(4, 4, 12, right=False)
    assert np.allclose(bb, [4., 6., 8., 10., 12.008])
    assert_raises(ValueError, cyclecount.getbins, 4, 3., 3.)
...
def test_sigcount_1():
    ...
    assert np.all(table.columns == ['(0.402, 49.500]',
                                    '(49.500, 98.500]'])
    assert np.allclose(table.values, [[12.5, 12.0],
                                      [12.5, 12.5]])
...
def test_sigcount_2():
    ...
    assert np.all(table.columns == ['[0.500, 49.500)',
                                    '[49.500, 98.598)'])
    assert np.allclose(table.values, [[12.0, 12.5],
                                      [12.5, 12.5]])
