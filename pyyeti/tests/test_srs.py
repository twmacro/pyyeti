...
def do_srs_sol(zdd, sr, freq, Q, ic='steady',
               stype='absacce', ppc=10, rolloff='fft',
               eqsine=False, time='primary', parallel='auto'):
    sh, resp = srs.srs(zdd, sr, freq, Q, getresp=True, ic=ic,
                       stype=stype, rolloff=rolloff, eqsine=eqsine,
                       time=time, parallel=parallel)
    sh1 = srs.srs(zdd, sr, freq, Q, ic=ic, peak='abs',
                  stype=stype, rolloff=rolloff, eqsine=eqsine,
                  time=time, parallel=parallel)
    shpos = srs.srs(zdd, sr, freq, Q, ic=ic, peak='pos',
                    stype=stype, rolloff=rolloff, eqsine=eqsine,
                    time=time, parallel=parallel)
    shneg = srs.srs(zdd, sr, freq, Q, ic=ic, peak='neg',
                    stype=stype, rolloff=rolloff, eqsine=eqsine,
                    time=time, parallel=parallel)
    shposs = srs.srs(zdd, sr, freq, Q, ic=ic, peak='poss',
                     stype=stype, rolloff=rolloff, eqsine=eqsine,
                     time=time, parallel=parallel)
    shnegs = srs.srs(zdd, sr, freq, Q, ic=ic, peak='negs',
                     stype=stype, rolloff=rolloff, eqsine=eqsine,
                     time=time, parallel=parallel)
    shrms = srs.srs(zdd, sr, freq, Q, ic=ic, peak='rms',
                    stype=stype, rolloff=rolloff, eqsine=eqsine,
                    time=time, parallel=parallel)
    sh0, resp0 = srs.srs(zdd[0], sr, freq, Q, getresp=True, ic=ic,
                         stype=stype, rolloff=rolloff, eqsine=eqsine,
                         time=time, parallel=parallel)
    sh01 = srs.srs(zdd[0], sr, freq, Q, ic=ic,
                   stype=stype, rolloff=rolloff, eqsine=eqsine,
                   time=time, parallel=parallel)
    return (sh, resp, sh1, shpos, shneg, shposs, shnegs,
            shrms, sh0, resp0, sh01)


def do_comp(stype, ic, sh, resp, sh1, shpos, shneg, shposs, shnegs,
...
def test_srs_absacce():
    ...
    for parallel in ['yes', 'no']:
        for ic in ['steady', 'zero']:
            ...
            (sh, resp, sh1, shpos, shneg, shposs, shnegs, shrms,
             sh0, resp0, sh01) = \
                do_srs_sol(zdd, sr, freq, Q, ic=ic,
                           stype=stype, parallel=parallel)
            # get solution via odeint:
            xdd, udd, ud, u, pacce, pvelo = do_odeint_sol(
                t, zdd, ic, w, B, K, nfreq)
            do_comp(stype, ic, sh, resp, sh1, shpos, shneg,
                    shposs, shnegs, shrms, sh0, resp0, sh01, xdd)
    ...
    for parallel in ['yes', 'no']:
        for ic in ['steady', 'zero']:
            ...
            do_comp(stype, ic, sh, resp, sh1, shpos, shneg, shposs,
                    shnegs, shrms, sh0, resp0, sh01, udd)
    for ic in ['shift', 'mshift']:
        # get solution via srs.srs:
        (sh, resp, sh1, shpos, shneg, shposs, shnegs, shrms,
         sh0, resp0, sh01) = \
...
def test_srs_relvelo():
    ...
    for parallel in ['yes', 'no']:
        for ic in ['steady', 'zero']:
            ...
            xdd, udd, ud, u, pacce, pvelo = do_odeint_sol(
                t, zdd, ic, w, B, K, nfreq)
        do_comp(stype, ic, sh, resp, sh1, shpos, shneg, shposs, shnegs,
                shrms, sh0, resp0, sh01, ud)
    for ic in ['shift', 'mshift']:
        # get solution via srs.srs:
        (sh, resp, sh1, shpos, shneg, shposs, shnegs, shrms,
         sh0, resp0, sh01) = \
            do_srs_sol(zdd, sr, freq, Q, ic=ic, stype=stype)
        # get solution via odeint:
        xdd, udd, ud, u, pacce, pvelo = do_odeint_sol(t, zdd, ic, w,
                                                      B, K, nfreq)
        do_comp(stype, ic, sh, resp, sh1, shpos, shneg, shposs, shnegs,
                shrms, sh0, resp0, sh01, ud)
    ...
    for parallel in ['yes', 'no']:
        for ic in ['steady', 'zero']:
            ...
            xdd, udd, ud, u, pacce, pvelo = do_odeint_sol(
                t, zdd, ic, w, B, K, nfreq)
            do_comp(stype, ic, sh, resp, sh1, shpos, shneg, shposs,
                    shnegs, shrms, sh0, resp0, sh01, pacce)
    for ic in ['shift', 'mshift']:
        # get solution via srs.srs:
        (sh, resp, sh1, shpos, shneg, shposs, shnegs, shrms,
         sh0, resp0, sh01) = \
    ...
    for parallel in ['yes', 'no']:
        for ic in ['steady', 'zero']:
            ...
            xdd, udd, ud, u, pacce, pvelo = do_odeint_sol(
                t, zdd, ic, w, B, K, nfreq)
            do_comp(stype, ic, sh, resp, sh1, shpos, shneg, shposs,
                    shnegs, shrms, sh0, resp0, sh01, pvelo)
    for ic in ['shift', 'mshift']:
        # get solution via srs.srs:
        (sh, resp, sh1, shpos, shneg, shposs, shnegs, shrms,
         sh0, resp0, sh01) = \
    ...
    for stype in ['absacce', 'relacce', 'relvelo',
        ...
        for time in ['primary', 'total', 'residual']:
            for ic in ['steady', 'shift', 'zero', 'mshift']:
                sh = srs.srs(zdd, sr, freq, Q, ic=ic,
                             stype=stype, time=time)
                shr, resp = srs.srs(zdd, sr, freq, Q, ic=ic,
                                    stype=stype, time=time,
                                    getresp=True)
                shm = srs.srs(zddm, sr, freq, Q, ic=ic,
                              stype=stype, time=time)
                shmr, respm = srs.srs(zddm, sr, freq, Q, ic=ic,
                                      stype=stype, time=time,
                                      getresp=True)
...
def test_srs_total():
    Q, freq, nfreq, j, k, sr, t, w, B, K, cut, zdd = get_params_2()
    ic = 'zero'
    for i, stype in enumerate(['absacce', 'relacce', 'relvelo',
                               'reldisp', 'pacce', 'pvelo']):
        # get solution via srs.srs:
        (sh, resp, sh1, shpos, shneg, shposs, shnegs, shrms,
         sh0, resp0, sh01) = \
        ...
        do_comp(stype, ic, sh, resp, sh1, shpos, shneg, shposs,
                shnegs, shrms, sh0, resp0, sh01, sol,
...
def test_srs_rolloff():
    ...
    sh2, resp = srs.srs(sig, sr, frq, Q, rolloff='prefilter', ppc=15,
                        getresp=True)
    assert np.all(sh1 == sh2)
    ...
    assert abs(sh1[:2]-sh4[:2]).max() < .005*abs(sh1[:2]).max()
    ...
    assert abs(sh1_fft-sh1).max() < .001*abs(sh1).max()
    assert abs(sh2_fft-sh2).max() < .001*abs(sh2).max()
    ...
    sh1, resp1 = srs.srs(sig, sr, frq, Q, rolloff=srs.fftroll,
                         peak=rmsmeth, getresp=True)
    sh2, resp2 = srs.srs(sig, sr, frq, Q, rolloff='fft', peak='rms',
                         getresp=True)
    assert np.all(sh1 == sh2)
...
def test_vrs():
    ...
    v3, m3 = srs.vrs(spec, frq, Q, linear=False, Fn=fn,
                     getmiles=True)
    assert np.all(v3 == v)
    assert np.all(m3 == m)

    v3b, m3b = srs.vrs(spec, frq, Q, linear=False, getmiles=True)
    v3c, m3c = srs.vrs(spec, frq, Q, linear=False, Fn=frq,
                       getmiles=True)
...
def test_srsmap():
    ...
    Q = 20
    mp, t, f = srs.srsmap(2, .5, sig, sr, frq, Q, .02, eqsine=1)
    assert np.all(f == frq)
    dt = 1.
